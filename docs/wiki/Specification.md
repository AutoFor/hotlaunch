...
| フィールド | 型 | デフォルト | 説明 |
|-----------|-----|-----------|------|
| `key` | string | `"F12"` | リーダーキー名（`Keys` 列挙体の名前。`"Alt"` も使用可） |
...
| モード | コマンドライン引数 | 出力内容 |
|--------|------------------|---------|
| 通常 | （なし） | 起動/終了、リーダーキー押下、アプリ起動/フォーカス |
| 詳細 | `--verbose` または `-v` | 上記 + 全キー押下イベント |
